...
function IconButton({ className, ...props }: IconButtonProps) {
  return (
    <ButtonPrimitive
      ...
      className={cn(
        "text-fg-2 hover-hover:hover:bg-hover hover-hover:hover:text-fg-1 motion-safe:ease-[var(--ease-out)] inline-flex size-8 shrink-0 cursor-pointer items-center justify-center rounded-md border border-transparent bg-transparent motion-safe:transition-[color,background-color,transform] motion-safe:duration-150 active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className,
      )}
      {...props}
    />
